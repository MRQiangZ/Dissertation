...
print("Last time for svr_12(epsilon = 0.8)")
...
#x_15 = X_train_15.iloc[index_list].values
"""
...
"""
"""
svr_15 = SVR(kernel = 'rbf',C=100,gamma = 1.0)
time_start = datetime.datetime.now()
print(time_start.strftime('%Y-%m-%d %H:%M:%S'))
svr_15.fit(x_15,y)
time_end = datetime.datetime.now()
...
print('*'*50)
"""
svr_12 = SVR(kernel = 'rbf',C=100,gamma = 1.0,epsilon=0.8)
time_start = datetime.datetime.now()
print(time_start.strftime('%Y-%m-%d %H:%M:%S'))
svr_12.fit(x_12,y)
joblib.dump(svr_12,'./model/svr_final_12.m')
time_end = datetime.datetime.now()
print('*'*10+'Finished'+'*'*10)
print(time_end.strftime('%Y-%m-%d %H:%M:%S'))
print('*'*50)
